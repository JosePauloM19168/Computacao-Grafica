...
#glm.lookAt ->define a posição da camera de acordo com a cena
#primeiro paramentro -> onde está a camera 
#segundo parametro -> onde ela esta olhando
#terceiro parametro -> direção pra onde a camera ta olhando

#math.pi/2 -> define a visao panoramica
    projection = glm.perspective(math.pi/2,900/600,0.1,100)  #0,2,8
    camera = glm.lookAt(glm.vec3(1,0,0),glm.vec3(0,0,0),glm.vec3(0,1,0))
    model = glm.rotate(a,glm.vec3(0,1,0))
    mvp = projection * camera * model
    #mvp-> aqui é fundido todas as transformações anteriores
